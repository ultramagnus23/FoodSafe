...
function DistrictInner() {
  ...
  if (!loggedIn) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-20 text-center">
        <p className="mb-4 font-medium text-ink">Sign in to view district risk reports.</p>
  ...
  if (risk.isError || !risk.data) {
    return <div className="mx-auto max-w-3xl px-6 py-20 text-center text-provenance">No data for this district/commodity.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <p className="mb-3 text-provenance">
        {r.state}
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          <div key={m.k} className="rounded-lg border border-line bg-porcelain p-4">
            <div className="font-display text-2xl font-semibold text-ink">{m.v}</div>
            <div className="mt-1 text-[11px] uppercase tracking-wide text-provenance">{m.k}</div>
          </div>
        ))}
      </div>
      ...
      {tab === "tests" && (
        <div className="overflow-x-auto rounded-lg border border-line">
          {r.enforcement_events.length ? (
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-porcelain text-left text-xs uppercase tracking-wide text-provenance">
                  <th className="sticky left-0 z-10 bg-porcelain px-4 py-3">Date</th>
              ...
              <tbody>
                {r.enforcement_events.map((e, i) => (
                  <tr key={i} className="border-t border-line">
                    <td className="sticky left-0 z-10 bg-slab px-4 py-3 font-mono text-xs">{e.test_date}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded bg-provenance-pale px-2 py-0.5 font-mono text-[11px] text-ink">
                        {e.source_type.toUpperCase()}
                      </span>
                      {e.source_url && (
                        <a href={e.source_url} target="_blank" rel="noreferrer" className="ml-1.5 text-xs text-ink">
                          ↗
                        </a>
                      )}
                    </td>
            ...
            <p className="p-6 text-provenance">No enforcement records found for this selection.</p>
          )}
        </div>
      )}
      ...
      {tab === "contaminants" && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          {tc.length ? (
            tc.map((c, i) => (
              <div key={i} className="rounded-lg border border-line bg-slab p-6">
                <div className="mb-1.5 font-semibold">{cap(c.name.replace(/_/g, " "))}</div>
                <div className="mb-1 font-mono text-2xl text-risk">{((c.fail_rate || 0) * 100).toFixed(1)}%</div>
                <div className="text-xs text-provenance">Failure rate</div>
              </div>
            ))
          ) : (
            <p className="text-provenance">No contaminant breakdown for this selection.</p>
          )}
        </div>
      )}
              ...
              {disease.data.rows.map((row, i) => (
                <div key={i} className="rounded-lg border border-line bg-slab p-6">
                  ...
                  {row.fssai_vs_codex_gap != null && row.fssai_vs_codex_gap > 1 && (
                    <div className="mt-2 rounded bg-caution-pale px-3 py-1.5 text-xs text-caution">
                      FSSAI&apos;s limit here is {row.fssai_vs_codex_gap.toFixed(1)}x more permissive than Codex
                      Alimentarius.
                    </div>
                  )}
                  <div className="mt-3 text-xs text-provenance">
      ...
      <div className="mt-10">
        <h2 className="mb-1 font-display text-2xl font-normal">Contamination Trend</h2>
        <p className="mb-4 text-sm text-provenance">Monthly mean PPB over time, with Mann-Kendall trend detection.</p>
